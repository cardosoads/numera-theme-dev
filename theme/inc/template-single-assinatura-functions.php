...
if ($post_meta !== "") {
    $nome_completo = $post_meta['_nome_completo'][0];
    $data_nascimento = $post_meta['_data_nascimento'][0];
}
...
//
//        if (count($datas) < 2 || stripos($datas[1], "Até o fim da vida") !== false) {
//            continue; // Ignora ciclos sem data final válida
//        }
//
//        // Pegando os anos das datas do ciclo
//        $anoInicio = (int) substr($datas[0], -4);
//        $anoFim = (int) substr($datas[1], -4);
//
//        // Calculando idade no início e no fim do ciclo
//        $idadeInicio = $anoInicio - (int) $anoNascimento;
//        $idadeFim = $anoFim - (int) $anoNascimento;
//
//        $idades[$nomeCiclo] = [
//            'Idade início' => $idadeInicio,
//            'Idade fim' => $idadeFim
//        ];
//    }
//
...
//echo '<pre>';
////var_dump($sequencias_positivas_options);
//var_dump($sequencias_negativas_options);
//echo '</pre>';
//die();
...
function getLetraNome($nome_completo, $primeira_letra) {
    ...
    $primeiraLetraNome = strtolower(substr($nomeCompleto, 0, 1)); // Extrai a primeira letra e a transforma em minúscula
    $resultadoLetra =[];
